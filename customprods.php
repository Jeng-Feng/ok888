<?php
  ...
  require_once($config['global_library'].'/_top_customprods.php');
?>
<!DOCTYPE html>
...
<!-- InstanceBeginEditable name="custom javascript" -->
  <? require_once($config['global_library'].'/_customprods.php'); ?>
...
<div id="bootstrap_content">
  <div class="container php-custom-prod-content">
    ...
    <div class="alert alert-danger" role="alert">
      <p class="alert-link">很抱歉,此分類下尚無任何產品!</p>
    </div>
    <?}?>
    
    <div class="row">
      <?foreach($customProducts as $k=>$v){?>
      <div class="col-md-3 col-sm-4 col-xs-6">
        <div class="panel panel-default php-custom-prod-grid">
          <div class="panel-body">
            <div class="php-custom-prod-imgbox">
              <a href="<?if(trim($v['link'])==''){?>customprod.php?c=<?=$linkpcid?>&p=<?=$v['linkpid']?><?}else{?><?=$v['link']?>" target="_blank<?}?>" title="<?=$v['productName']?>">
                <img class="img-responsive" src="<?=$customProductImgBase?>/<?=$v['proPic2']?>" alt="<?=$v['productName']?>" />
              </a>
            </div>
            <div class="php-custom-prod-text php-custom-prod-name"><?=$v['productName']?></div>
            <div class="php-custom-prod-text php-custom-prod-desc"><?=strip_tags($v['productDesc'])?></div>
            <div class="php-custom-prod-text php-custom-prod-more">
              <div class="php-custom-prod-list-price">售价 $ <?=$v['price']?></div>
            </div>
          </div>
          <div class="panel-footer">
            ...
            <?if($v['editor']){?>
            <a href="<?if(trim($v['link'])==''){?>customprod.php?c=<?=$linkpcid?>&p=<?=$v['linkpid']?><?}else{?><?=$v['link']?>" target="_blank<?}?>" title="<?=$v['productName']?>">
              <div class="btn btn-primary btn-small php-custom-prod-addcart">查看商品明細</div>
            </a>
            <?}else{?>
            <div class="btn btn-primary btn-small php-custom-prod-addcart" id="<?=$v['productID']?>">加入購物車</div>
            <?}?>
            <?}?>
          </div>
        </div>

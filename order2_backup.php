			...
			<div class="text-left">
				<form name="FileHeaderForm" class="form-horizontal" method="post" id="christest" action="./order2.php" >
					<h5 class="order2-title">
						...
						訂購產品、數量與價格
						<span class="badge pull-right"><?=count($orderlist_data)?></span>
					</h5>
							...
							<div class="panel panel-primary sublist ol<?=$data["index"]?>">
								<div class="panel-heading">
									<h6 class="order-title">
										...
										<?endif;?>
										<?if(!isset($data["cpid"])):?>
											<span class="delorder glyphicon glyphicon-remove pull-right canClick" title="取消訂購" onclick="deloitem(<?=$data["index"]?>,this,'<?=$data["bookid"]?>');"></span>
										<?else:?>
											<span class="delorder glyphicon glyphicon-remove pull-right canClick" title="取消訂購" onclick="deloitem(<?=$data["index"]?>,this,'<?=$data["cpid"]?>');"></span>
										<?endif;?>
									</h6>
								</div>
								<div class="panel-body">
									<div class="row">
										...
										<div class="sl-productinfor col-md-8">
											<?if(empty($data["warningmessage"])):?>
												<div class="row order2-form-row">
													...
													<div class="pi-context col-xs-3 col-md-2">
														<?if($data["allowchangeunit"]):?>
															<?if(!isset($data["cpid"])):?>
																<select class="amountchoose form-control input-sm" name="amountchoose[]" onchange="modoitem(<?=$data["index"]?>,this);">
															<?else:?>
																<select class="customProd-qty cpq<?=$data["index"]?> form-control input-sm" name="amountchoose[]" onchange="changeCustomProdShopQty(<?=$data["index"]?>,this);">
															<?endif;?>
												...
												<?if(count($data["poption"]) > 0):?>
													<div class="row spec order2-form-row">
														...
														<?foreach($data["poption"] as $poption):?>
															<div class="pi-context col-xs-9 col-md-10">
																<span><?=$poption?></span>
															</div>
														<?endforeach?>
												...
												<div class="row">
													<?if($data["bookid"] != "customerproduct"):?>
														<div class="col-md-3">
															<button class="btn btn-default btn-sm" onclick="reedit('<?=$data["bookid"]?>','<?=$data["editorclass"]?>');" >繼續編輯該產品</button>
														</div>
													<?endif;?>
													<?if(!isset($data["cpid"])):?>
														<div class="col-md-3">
															<button class="btn btn-default btn-sm" onclick="detaitem(<?=$data["index"]?>);">修改訂購細節</button>
														</div>
													<?endif;?>
												</div>
											<?else:?>
							...
							<div class="row">
								...
								<div class="text-center col-xs-12 col-sm-12 col-md-6">		
									<a href="./booklists.php" class="">
										<img src="./images/im/addcart.gif" border="0" />加購其他製作項目
									</a>							
								</div>	
							</div>
						</div>	
							...
							<? foreach($shipmentlist_data as $data): ?>
								<div class="panel panel-info sublist sl<?=$data["value"]?>">
									<div class="panel-heading">
										...
										<span class="freight pull-right">
											<?=$order["symbol_left"]?>
											<?=$data["showfreight"]?>
											<?=$order["symbol_right"]?>
										</span>
										<span class="freefreight pull-right">
											<?=$order["symbol_left"]?>
											0
											<?=$order["symbol_right"]?>
										</span>
									</div>
									<div class="panel-body csdiv <?=$data["containerclassname"]?>" <?=$data["containeridname"]?>>
					...
					<div class="invoicearea row form-group" id="php_invoice2">
						<div class="col-xs-6 col-sm-2 col-md-2">
							<label class="form-control">
								<input type="radio" name="php_invoicetype" value="2" />二聯發票
							</label>
						</div>
						<div class="col-xs-6 col-sm-2 col-md-2">
							<label class="form-control">
								<input type="radio" name="php_invoicetype" value="3" />三聯發票
							</label>
						</div>
						<div id="php_invoicearea" class="col-xs-12 col-sm-8 col-md-8">
					...
					<div class="smsarea row" id="php_sms">
						...
						<div id="php_smsnotifyinput" class="col-xs-12 col-sm-12 col-md-6">
							<input type="text" class="form-control" maxlength="10" value="" name="php_smsnotifynum" id="php_smsnotifynum" title="請輸入手機號碼"  placeholder="請輸入手機號碼" />
						</div>
					</div>
					<h5 class="order2-title">
						<span class="glyphicon glyphicon-tag"></span>
					...
					<div class="text-left paymentlist">

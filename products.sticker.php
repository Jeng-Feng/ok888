...
<? require_once($config['global_library'].'/_top_products.sticker.php'); ?>
<!DOCTYPE html>
...
<? require_once($config['global_library'].'/_products.sticker.php'); ?>
<script type="text/javascript">
//<![CDATA[
	$(document).ready(function() {
	});
	function singletemplatefunc(template){
		//alert(template);
...
<div id="bootstrap_content">	
        ...
        <div class="row">
        <div class="col-sm-8">
        <img class="img-responsive center-block"  src="/images/step1_title.png" alt="請選擇產品"/>
        </div>
        </div>
        </div>
        </div>
...
?>
	...
	<div class="container">
		...
		<div class="php_productarea">
            ...
            <article class="col-sm-3 isotopeItem women php_pcpli php_pcpli<?=$data['pcssname']?> canClick" onclick="showtemplate('<?=strtolower($data['productID'])?>','<?=$data['forthistemplateclass']?>')" title="<?=$data['productName']?>">
									...
									<section class="boxContent box_heigh">
										<h3 class="php_ptitle text-left"><?=$data['productName']?></h3>
                                        <p class="php_pprice text-left"><?=$data['pprice']?> <?=$data['lang_dollar_Unit']?> <?=$data['setQty']?></p>
										<p class="php_pdescribe text-left"><?=$data['intro']?></p>
                                        <div class="php_pgettemplate center-block" title="<?=$data['lang_chooseTemplate']?>"><?=$data['lang_chooseTemplate']?></div>
									</section>
        ...
        <div class="row">
        <div class="col-sm-8">
        <img class="img-responsive center-block"  src="/images/step3_title.png" alt=" 開始"/>
        </div>
        </div>
        </div>
        </div>

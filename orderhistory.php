<?php
  ...
  require_once($config['global_library'].'/_top_orderhistory.php');
?>
<!DOCTYPE html>
...
<!-- InstanceBeginEditable name="custom javascript" -->
  <?require_once($config['global_library'].'/_orderhistory.php');?>
...
<div id="bootstrap_content">
  <div class="container" align="center">
    ...
    <div class="php-orderhistory">
      
      <div class="row">
        <div class="col-md-6 col-sm-6 col-xs-12">
          <div>查詢期間：</div>
          <input id="startdate" name="startdate" type="text" readonly="readonly" value="<?=$startdate?>">
          <span>～</span>
          <input id="enddate" name="enddate" type="text" readonly="readonly" value="<?=$enddate?>">
        </div>
        <div class="col-md-6 col-sm-6 col-xs-12 php-orderhistory-search-btnbox">
          <input type="button" class="btn btn-primary btn-small sendsearch" value="送出查詢" onclick="checkorder();" />
          <input type="button" class="btn btn-primary btn-small cleardate" value="清除日期" onclick="document.location.href='orderhistory.php';" />
        </div>
      </div>
      
      <div class="table-responsive">
        <table class="table table-bordered">
          <tr class="php-orderhistory-title">
            <th>訂單流水號</th>
            <th>下訂日期</th>
            <th>產品</th>
            <th>規格</th>
          ...
          <?if($orderhistories==0){?>
          <tr class="noorder">
            <td colspan="7" align="center">無訂單</td>
          </tr>
          <?}else{?>
          <?foreach($data as $k=>$v){?>
          <tr class="content <?=$v['ordercss']?>">
            <?=$v['orderID']?>
            <?=$v['orderDatetime']?>
            <td class="ol3">
              <?=$v['productName']?>
      ...
      <!-- 分頁器 -->
      <?=pagelinkfunc($page,$totalorder,$pagelink,$perpagecount,$totalpage)?>
      <!-- 分頁器 -->
    </div>

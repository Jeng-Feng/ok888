...
<? require_once($config['global_library'].'/_top_products.photobook.php'); ?>
<!DOCTYPE html>
...
<!-- InstanceBeginEditable name="custom javascript" -->
  <? require_once($config['global_library'].'/_products.photobook.php'); ?>
  <!-- InstanceEndEditable -->
...
<div id="bootstrap_content">
  <div class="container pt40 pb40">
    <div class="product-step1">
      <div class="row">
        <div class="col-sm-6"> <img class="img-responsive center-block"  src="/images/step1_title.png" alt="請選擇產品"/> </div>
      </div>
    </div>
  </div>
  <?php
  ...
  <div class="container">
    ...
    <div class="php_productarea">
      <div class="row">
        ...
        <article class="col-sm-3 isotopeItem women php_pcpli php_pcpli<?=$data['pcssname']?> canClick" onclick="showtemplate('<?=strtolower($data['productID'])?>','<?=$data['forthistemplateclass']?>')" title="<?=$data['productName']?>">
          <figure> <span class="php_pimgarea"><img class="img-responsive center-block" src="<?=$data['thumbnail_url']?>" alt="<?=$data['productName']?>" /></span> </figure>
          <section class="boxContent box_heigh">
            <h3 class="php_ptitle text-left">
            <?=$data['productName']?>
            </h3>
            <p class="php_pprice text-left">
              <?=$data['pprice']?>
              <?=$data['lang_dollar_Unit']?>
              <?=$data['setQty']?>
            </p>
            <p class="php_pdescribe text-left">
              <?=$data['intro']?>
            </p>
            <div class="php_pgettemplate center-block" title="<?=$data['lang_chooseTemplate']?>">
              <?=$data['lang_chooseTemplate']?>
            </div>
          </section>
        </article>
        <? endforeach; ?>
        
        <!--old <? foreach($products_data as $data): ?>
        <div class="col-md-12 php_pcpli php_pcpli<?=$data['pcssname']?> canClick" onclick="showtemplate('<?=strtolower($data['productID'])?>','<?=$data['forthistemplateclass']?>')" title="<?=$data['productName']?>">
          <div class="row">
            <div class="col-md-3">
      ...
      <div class="php_showallproduct" style="display:none;" title="<?=$lang_ClickToExpandProduct?>" onclick="showallproduct()">
        <?=$lang_ClickToExpandProduct?>
      </div>
    </div>
    <div class="container pt40 pb40">
      <div class="product-step1">
        <div class="row">
          <div class="col-sm-6"> <img id="php_step2" class="img-responsive hidden" src="<?=$config["url"]?>/images/step2_title.png" alt="<?=$lang_PlsChooseTemplateclass?>" /> </div>
        </div>
      </div>
    </div>
    <br />
    ...
    <div class="php_templatearea php_templatearea<?=$productID?>"  style="display:none;">
      <div class="row">
        ...
        <div class="col-md-4 col-sm-6"> <span class="php_taimgarea canClick" onclick="gotoeditorfunc('<?=$data['productID']?>','<?=$data['key2']?>','<?=$data['loginstatus']?>','<?=$data['previewcheck']?>')"> <img src="<?=$data['thumbnail_url']?>" /> </span>
        <? if($data['has_preview'] == true): ?>
        <span class="php_showpreview" onclick="fallrpreview('<?=$data['preview']?>','<?=$data['productName']?>')" title="<?=$data['lang_shareCompositions']?>">
          <?=$data['lang_shareCompositions']?>
        </span>
        <? endif; ?>
      </div>
      <? endforeach; ?>
  ...
  <div class="row keyininfor" style="display:none">
    <div class="container pt40 pb40">
      <div class="product-step1">
        <div class="row">
          <div class="col-sm-6"> <img class="img-responsive center-block"  src="/images/step3_title.png" alt=" 開始"/> </div>
        </div>
      </div>
    </div>
    <br/>
    <div class="col-md-6">
      <form class="form-horizontal" role="form"  method="post" name="gotoeditor" id="gotoeditor">
        ...
        <div class="form-group">
          <label for="inputbksubtitle3" class="col-sm-4 control-label">副標題：</label>
          <div class="col-sm-8">
            <input type="text" class="form-control" id="bksubtitle" name="bksubtitle" placeholder="" maxlength="64" >
          </div>
        </div>
        <div class="form-group">
          <label for="inputbksubtitle3" class="col-sm-4 control-label">作者：</label>
          <div class="col-sm-8">
            <input type="text" class="form-control" id="bkauthor" name="bkauthor" placeholder="" maxlength="24">
          </div>
        </div>
        <div class="form-group">
          <label for="inputbksubtitle3" class="col-sm-4 control-label">書背：</label>
          <div class="col-sm-8">
            <input type="text" class="form-control" id="bkspine" name="bkspine" placeholder="" maxlength="36">
          </div>
        </div>
        <div class="form-group text-center">
          <div id="startbt" class="col-sm-offset-3 col-sm-10">
    ...
    <div class="col-md-6 kipreview"> </div>
  </div>
</div>
</div>

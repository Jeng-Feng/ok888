...
$(function(){
	...
	if (iQty == 0){
		 ...
		 $('#all_my_shoppingcart').removeattr('onclick'); //當購物車清單數量=0時，動態移除結帳函式gotocounter();
	} 
		
	$('.addToCart').click(function() {
		setTimeout(function(){
			$('#CartQty').html($('#shoppingcart_qty').html());
			$('#all_my_shoppingcart').attr('onclick','gotocounter()'); //當購物車清單數量>0時，動態加入結帳函式gotocounter();
		},300);
	});
									...
									<div class="box-middle">
										...
										<div class="row">
											<div class="addcartbutton col-md-6 col-sm-6 col-xs-6">
												<button class="addToCart otheredit btn btn-default btn-sm php-booklists-editorbutton" title="加入購物車" onclick="addcart('<?=$data["b_id"]?>',this,'<?=$data["bk_name"]?>');">
												<span class="glyphicon glyphicon-shopping-cart"></span>
												加入購物車
												</button>
											</div>
											<div class="removecartbutton col-md-6 col-sm-6 col-xs-6">
												<button class="removeFromCart otheredit btn btn-default btn-sm php-booklists-editorbutton" title="已加入購物車，移出購物車" onclick="delitem(this,'<?=$data["delitemmode"]?>')" bookID="<?=$data["b_id"]?>" customProductID="<?=$data["checkbookidincart"]["customProdKey"]?>">
